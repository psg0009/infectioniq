...
def _build_dispenser_response(disp: Dispenser, status: DispenserStatus) -> DispenserStatusResponse:
    """Build a DispenserStatusResponse from a Dispenser and its status."""
...
async def list_dispensers(db: AsyncSession = Depends(get_db)):
    ...
    return [_build_dispenser_response(disp, status) for disp, status in rows]
...
async def get_dispenser_status(dispenser_id: str, db: AsyncSession = Depends(get_db)):
    ...
    disp, status = row
    return _build_dispenser_response(disp, status)
